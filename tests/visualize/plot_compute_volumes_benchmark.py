"""
Plot compute_volumes benchmark: poly_ratio (x) vs time (y) line chart.

Reads pytest-benchmark JSON and plots test_compute_volumes_100x100_benchmark
...
this script manually for volumes, or add it to the Makefile.

Standalone:

    uv run --group visualize python \
        tests/visualize/plot_compute_volumes_benchmark.py \
        [path/to/benchmark.json]

Output: tests/outputs/visualize/compute_volumes_benchmark.html (optional .png).
"""
...
def load_series(json_path: Path) -> tuple[list[int], list[float]]:
    ...
    compute_volumes 100x100 benchmark.
    """
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot compute_volumes 100x100 benchmark: poly_ratio vs time"
    )
    parser.add_argument(
        "json_path",
        nargs="?",
    ...
    parser.add_argument(
        ...
        default=None,
        help="Output path \
            (default: tests/outputs/visualize/compute_volumes_benchmark.html)",
    )
    parser.add_argument(
        "--png",
        action="store_true",
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(
            title="cell_volumes (100x100 volumes): poly_ratio vs mean time",
    ...
    out_html = args.output or (OUT_DIR / "compute_volumes_benchmark.html")
