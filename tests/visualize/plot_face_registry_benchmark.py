"""
Plot build_face_registry benchmark: poly_ratio (x) vs time (y) line chart.

Reads pytest-benchmark JSON and plots test_build_face_registry_100x100_volumes
results. When using `make benchmark`, the Makefile writes benchmark JSON to
tests/outputs/benchmark/latest.json and then runs this script with no args.

Standalone:

    uv run --group visualize python \
        tests/visualize/plot_face_registry_benchmark.py [path/to/benchmark.json]

Output: tests/outputs/visualize/face_registry_benchmark.html (optional .png).
"""
...
# Used by `make benchmark`; pytest writes here via --benchmark-json.
...
GROUP_NAME = "test_build_face_registry_100x100_volumes"
...
def load_series(json_path: Path) -> tuple[list[int], list[float]]:
    ...
    face registry 100x100 volumes benchmark.
    """
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot build_face_registry benchmark: poly_ratio vs time"
    )
    parser.add_argument(
        "json_path",
        nargs="?",
    ...
    parser.add_argument(
        ...
        default=None,
        help="Output path \
            (default: tests/outputs/visualize/face_registry_benchmark.html)",
    )
    parser.add_argument(
        "--png",
        action="store_true",
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(
            title="build_face_registry: \
                poly_ratio vs mean time (100x100 volumes)",
            xaxis={"title": "poly_ratio (%)", "dtick": 10},
            yaxis={"title": "Time (s)", "tickformat": ".3f"},
    ...
    out_html = args.output or (OUT_DIR / "face_registry_benchmark.html")
