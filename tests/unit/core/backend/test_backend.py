...
class TestBackendEinsum:
    ...
    def test_einsum(
        ...
        actual = backend.einsum(pattern, *tensors, dimension="auto")
        desired = np.einsum(pattern, *np_arrays)

        np.testing.assert_almost_equal(
            actual.to("cpu").numpy(), desired, decimal=5
        )
        ...
        if isinstance(backend, PhlowerBackend):
            if desired_dimension is None:
                ...
                assert actual.dimension.to(
                    "cpu"
                ) == pt.phlower_dimension_tensor(desired_dimension)
    ...
    def test_einsum_raises_unexpected_input_type(
        self, pattern: str, shapes: tuple[tuple[int]], backend: Backend
    ):
        np_arrays = [np.random.randn(*shape) for shape in shapes]
        with pytest.raises(ValueError, match="Unexpected tensor"):
            backend.einsum(pattern, np_arrays)
    ...
    def test_rearrange(
        ...
        actual = backend.rearrange(tensor, pattern=pattern)
    ...
    def test_rearrange_raises_unexpected_input_type(
        self, pattern: str, shape: tuple[int], backend: Backend
    ):
        np_array = np.random.randn(*shape)
        with pytest.raises(ValueError, match="Unexpected tensor"):
            backend.rearrange(np_array, pattern)
